"""Natural Earth ocean groups with a detailed, validated water footprint.

Group membership and output schema are retained from the original notebooks.
See ocean_partition.py and docs/ocean_method.rst for the corrected assignment.
"""

import geopandas as gpd
import pandas as pd
from shapely.geometry import box
from .ocean_partition import ocean_domain, partition_ocean
import logging
...
def generate_oceans(ne_geo_marine_polys_path, ne_geo_ocean_path, codes_id_path, oceans_from_ne_path,
    ...
    Supply countries_path to remove overlap with the prepared country coastline;
    the standard pipeline always does this. Without it, the repaired source ocean
    defines the footprint. No country polygons are changed.
    """
    ...
    if gdf_marine["parent_reg"].isna().any():
        missing = gdf_marine.loc[gdf_marine["parent_reg"].isna(), "name"].tolist()
        raise ValueError(f"Unmapped marine names: {missing}")
    ...
    countries = gpd.read_file(countries_path) if countries_path is not None else None
    water = ocean_domain(gdf_ocean, countries)
    final_oceans = partition_ocean(water, gdf_marine)
