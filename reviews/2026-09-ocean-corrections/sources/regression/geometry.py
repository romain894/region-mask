...
from .area import area_measurement

from .common import plain

AREA_METHOD = (
    "WGS84 surface-Jacobian integration over straight EPSG:4326 edges; signed "
    "fan triangles with 8/16-order quadrature error estimate plus roundoff. "
    "No seam wrapping, geometry repair, sliver deletion or geodesic reinterpretation. "
    "Uncertainty is numerical only, not a certified bound or source accuracy."
)


def area_km2(geometry):
    return area_measurement(geometry)['area_km2']
...
def compare_geometry(a, b):
    ...
    if valid:
        added_measure = area_measurement(None if spatial else b.difference(a))
        removed_measure = area_measurement(None if spatial else a.difference(b))
        added, removed = added_measure['area_km2'], removed_measure['area_km2']
        result.update(added_km2=added, removed_km2=removed,
                      symmetric_difference_km2=added + removed,
                      net_transfer_km2=added - removed,
                      change_numerical_uncertainty_km2=(
                          added_measure['area_numerical_uncertainty_km2'] +
                          removed_measure['area_numerical_uncertainty_km2']))
    else:
