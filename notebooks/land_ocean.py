...
def _(mo):
    mo.md("""
    # Prepare the land/ocean product

    Run country preparation first. Land is dissolved from those countries; ocean comes from the original Natural Earth ocean file.
...
def _(generate, mo, root, run_stage, settings):
    ...
    result = run_stage("land_ocean", root=root, settings=settings)
    return (result,)
